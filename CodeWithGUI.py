...
def scnlivecamera():
    ...
    while condn:
        ...
        cont = file.read()
        if cont == "working":
            setvar("Press Space to Scan for Text!","green")
            condn = False
...
win = tk.Tk()
win.geometry("350x135")
win.title("KickTheHomeworks")
...
tk.Label(win, text="@ Build by Ankit and Raghs").grid(row=5)
...
import string
import time
ip_add = "192.168.1.232:8080"    ###Enter your ip address here###
switch = True
file = open("zanotherankit.txt","w")
file.write("working")
file.close()
